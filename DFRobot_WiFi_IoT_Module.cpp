...
uint8_t DFRobot_WiFi_IoT_Module_I2C::publish(char *topic, String data)
{
  ...
  while(1){
    ...
    if(getMQTTSendState() == 1){
      ret = 0;
      break;
    }else if((currentTime-startingTime) > 10000){
      ret = -1;
      break;
    }
  }
...
char* DFRobot_WiFi_IoT_Module_I2C::HTTPGet(char *url)
{
  ...
  if(!parameterReturn(HTTP_NORMAL_RETURN, HTTP_ERROR_RETURN, &recvHTTPData[0])){
...
char* DFRobot_WiFi_IoT_Module_I2C::getVersion()
{
  ...
  delay(10);
  if(!parameterReturn(GET_VERSION, 0xff, &getVersionData[0])){
    DBG("Parameter return failed");
    return;
  }
...
uint8_t DFRobot_WiFi_IoT_Module_I2C::loop()
{
  ...
  if(readReg(IOT_COMMAND_REGTSTER, &buffer, 3) != 3){
    ...
    if(_callback){
      if(getData(READ_DATA_REGISTER, data, sizeof(uint8_t)*datalen) != (sizeof(uint8_t)*datalen)){
        DBG("READ WIFI_IOT_ERROR!!!!!!");
        return WIFI_IOT_ERROR;
      }else{
...
uint8_t DFRobot_WiFi_IoT_Module_I2C::thingSpeakSendMessage(char* data1, char *data2, char *data3)
{
  ...
  sendData += data3;
  sendData += "\r\0";
  HTTPGet((uint8_t*)sendData.c_str());
  return 1;
}
...
bool DFRobot_WiFi_IoT_Module_I2C::parameterReturn(uint8_t config, uint8_t config1, uint8_t *pBuf)
{
  bool ret = false;
  uint8_t buffer[2];
  if(readReg(IOT_COMMAND_REGTSTER, &buffer, 2) != 2){
    ...
    if(buffer[0] == config){
      ...
      if(getData(READ_DATA_REGISTER, data, sizeof(uint8_t)*datalen) != (sizeof(uint8_t)*datalen)){
        ...
        for(uint8_t i =0; i < sizeof(uint8_t)*datalen ; i++){
          pBuf[i] = data[i];
        }
        pBuf[sizeof(uint8_t)*datalen]= '\0';
        free(data);
        ret = true;
      }
